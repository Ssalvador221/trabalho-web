...
function PerfilForm() {
  ...
  const onSubmit = async (data) => {
    try {
      const emailResponse = await updateUserEmail(userId, {
        email: data?.email,
      });
      const nameResponse = await updateUserName(userId, {
        full_name: data?.full_name,
      });
      const phoneResponse = await updateUserPhone(userId, {
        phone: data?.phone,
      });
      console.log("Responses", emailResponse, nameResponse, phoneResponse);
      if (
        emailResponse.status === 204 &&
        nameResponse.status === 204 &&
        phoneResponse.status === 204
      ) {
        toast({
          variant: "default",
          title: "Sucesso!",
  ...
  return (
    <Card className="w-full h-full px-20 pb-20 pt-10 bg-gray-100">
      <form
        className="bg-white border-2 rounded-2xl w-3/4 ml-2 mr-0 p-10 flex flex-col items-center h-full"
